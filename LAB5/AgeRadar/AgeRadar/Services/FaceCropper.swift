...
public extension FaceCropper where T: CIImage {
  
  func crop(_ completion: @escaping (FaceCropResult<CIImage>) -> Void) {
    guard #available(iOS 11.0, *) else {
      return
    }
    ...
    self.detectable.cgImage!.face.crop { result in
      switch result {
      case .success(let cgFaces):
        let faces = cgFaces.map { cgFace -> CIImage in
          return CIImage(cgImage: cgFace)
        }
        completion(.success(faces))
      case .notFound:
        completion(.notFound)
