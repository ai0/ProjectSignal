...
class Application(tornado.web.Application):
    def __init__(self):
        ...
        handlers = [
            (r"/[/]?", BaseHandler),
            (r"/Signup[/]?", skh.SignupHandler),
            (r"/Login[/]?", skh.LoginHandler),
            (r"/Logout[/]?", skh.LogoutHandler),
            (r"/Handlers[/]?", skh.PrintHandlers),
            (r"/AddDataPoint[/]?", skh.UploadLabeledDatapointHandler),
            (r"/UpdateModel[/]?", skh.UpdateModelForUser),
        ...
        try:
            ...
            print(
                'Could not initialize database connection, stopping execution')
        ...
        settings = {
            'debug': True,
            'cookie_secret': 'R%~Z^_><L[/6+2<bwo4v',
            "login_url": "/Login"
        }
